...
const EnhancedAnalyticsFooter: React.FC = () => {
  ...
  const getDistanceLabel = (distance: number) => {
    if (distance < 50) return 'Local (< 50mi)';
    if (distance < 200) return 'Regional (50-200mi)';
    if (distance < 500) return 'Provincial (200-500mi)';
    return 'National/International (500mi+)';
  };

  return (
    <Card className="fixed bottom-0 left-0 right-0 z-40 bg-white border-t-2 border-sawmill-orange shadow-lg">
      <div className="px-6 py-3">
        {/* Compact View - Always Visible Smart Summary */}
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-8">
            <div className="flex items-center space-x-2">
              ...
              {stats?.totalPageViews && getTrendIcon(stats.totalPageViews, 100)}
            </div>
              ...
              <span className="text-sm font-medium text-sawmill-dark-brown">
                {isLoading ? '...' : (
                  stats?.topCountries.length ? 
                    `${getCountryFlag(stats.topCountries[0].country)} ${stats.topCountries[0].country} (${stats.topCountries[0].percentage}%)` :
                    '🌍 No data'
                )}
              </span>
            </div>
        ...
        {/* Expanded Intelligent Analytics Dashboard */}
        {isExpanded && (
              ...
              <TabsContent value="overview" className="mt-4">
                <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                  ...
                  <div className="bg-green-50 p-4 rounded-lg border border-green-200">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-xl font-bold text-green-800">{stats?.uniqueVisitors || 0}</p>
                        <p className="text-xs text-green-600">Quality: High</p>
                      </div>
                  ...
                  <div className="bg-purple-50 p-4 rounded-lg border border-purple-200">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-xs text-purple-600">Engaged users</p>
                      </div>
                  ...
                  <div className="bg-orange-50 p-4 rounded-lg border border-orange-200">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-xl font-bold text-orange-800">{stats?.bounceRate || 0}%</p>
                  ...
                  <div className="bg-indigo-50 p-4 rounded-lg border border-indigo-200">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-xs text-indigo-600 font-medium">Locations</p>
                        <p className="text-xl font-bold text-indigo-800">{stats?.topCountries.length || 0}</p>
                        <p className="text-xs text-indigo-600">Countries</p>
                      </div>
            ...
            {(!stats?.totalPageViews || stats.totalPageViews === 0) && (
              <div className="mt-4 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                <div className="text-sm text-yellow-800">
                  <strong>🚀 Analytics System Ready!</strong> 
                  <div className="mt-1 text-xs">
                    Your enhanced analytics with location tracking is now active. 
                    Visit your site to start seeing detailed visitor intelligence here.
                  </div>
                </div>
              </div>
            )}
