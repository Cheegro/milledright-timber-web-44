...
const Blog = () => {
  ...
  useEffect(() => {
    const filtered = posts.filter(post => post.title.toLowerCase().includes(searchQuery.toLowerCase()) || post.excerpt && post.excerpt.toLowerCase().includes(searchQuery.toLowerCase()));
  ...
  return <div className="min-h-screen flex flex-col">
      <SEOHead title="Blog & Workshop Journal" description="Insights, tips, and stories from our sawmill and woodworking projects. Stay updated with the latest techniques and project showcases." keywords="woodworking blog, sawmill tips, lumber insights, workshop journal" />
      
      <Header />
      
      <main className="flex-1">
        <BlogHeader title="Workshop Journal" description="Insights, tips, and stories from our sawmill and woodworking projects" showSearch={true} searchQuery={searchQuery} onSearch={setSearchQuery} />
        ...
        <div className="container-wide py-12">
          {loading ? <div className="flex justify-center py-20">
              ...
              {filteredPosts.map(post => <Card key={post.id} className="overflow-hidden hover:shadow-xl transition-all duration-300 border-0 bg-white shadow-lg hover:transform hover:scale-105">
                  {post.image_url && <div className="aspect-video relative overflow-hidden">
                  ...
                  <CardContent className="p-6">
                    ...
                    {post.blog_categories && <Badge variant="secondary" className="mb-3 bg-sawmill-orange/10 text-sawmill-orange border-sawmill-orange/20 bg-stone-900 rounded-none">
